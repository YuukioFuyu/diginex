            ...
            <form action="{{ route('login') }}" method="post" autocomplete="off" novalidate>
                ...
                <div class="form-footer">
                    <button type="submit" class="btn btn-primary w-100">
                        <x-svg i="login"/> {{ __('Sign in') }}</button>
                </div>
            </form>
        </div>

        @if(config('services.google.status') || config('services.facebook.status'))
            <div class="hr-text">{{ __('or') }}</div>
            <div class="card-body">
                <div class="row">
                    @if(config('services.facebook.status'))
                        <div class="col"><a href="{{ route('social.login','facebook') }}" class="btn w-100">
                                <x-icon name="facebook" class="me-1"/>
                                {{ __('Login with Facebook') }}
                            </a>
                        </div>
                    @endif

                    @if(config('services.google.status'))
                        <div class="col"><a href="{{ route('social.login','google') }}" class="btn w-100">
                                <x-icon name="google" class="me-1"/>
